...
// --- Health + Files + Upload (kept from starter kit) ---
...
export async function createRealtimeSession() {
  return apiFetch<RealtimeSessionToken>("/realtime/session", {
    method: "POST",
...
export async function invokeTool(request: ToolCallRequest) {
  return apiFetch<ToolCallResponse>("/tools/invoke", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
export async function finalizeCall(request: CallFinalizeRequest) {
...
export async function getCalls(limit = 100) {
  return apiFetch<Call[]>(`/calls?limit=${limit}`);
}

export async function getCallStats() {
  return apiFetch<CallStats>("/calls/stats");
}

export async function getCallActivity(days = 7) {
  return apiFetch<DailyCallCount[]>(`/calls/stats/activity?days=${days}`);
}
...
export async function getCallAudioUrl(callId: string) {
  return apiFetch<{ url: string }>(`/calls/${callId}/audio`);
}

export async function deleteCall(callId: string) {
  return apiFetch<{ deleted: boolean; call_id: string; keys_removed: number }>(
    `/calls/${callId}`,
    { method: "DELETE" },
  );
}
