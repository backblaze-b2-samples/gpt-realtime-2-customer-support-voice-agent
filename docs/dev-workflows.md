...
The Realtime API is intentionally **not** exercised in unit tests — it's a streaming, stateful, paid endpoint. Instead:

- **Repo layer (`services/api/app/repo/openai_client.py`)** — the only module that imports `openai`. Tests stub out `mint_realtime_session` and `generate_summary` with `monkeypatch.setattr`, returning canned `RealtimeSessionToken` / string payloads.
- **Service layer (`call_orchestrator.py`, `tool_executor.py`)** — tested end-to-end against the stubbed repo. Verifies bundle assembly, tool-trace ordering, and the order in which objects are written to B2.
- **Runtime layer (`/realtime/session`, `/tools/invoke`, `/calls/*`)** — tested via FastAPI's `TestClient`. The repo is monkeypatched at module import time in `tests/conftest.py`.
- **Frontend (`apps/web/e2e/call-smoke.spec.ts`)** — Playwright intercepts the `POST /realtime/session` request and returns a fake token. The actual `RTCPeerConnection` open is mocked so the UI flow (Start Call -> Connected -> End Call -> bundle shows in `/calls`) is exercised without contacting OpenAI.

If you need to exercise the real Realtime endpoint locally, set `OPENAI_API_KEY` in your `.env` and run `pnpm dev` — the dev server uses the real repo.
