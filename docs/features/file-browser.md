...
## Purpose
List, preview, download, and delete files stored in Backblaze B2. This is the **full-bucket operator view** — it shows every object regardless of prefix. The **sample-specific** view scoped to call bundles lives at `/calls` (see [Calls Explorer](calls-explorer.md)); use this view when you need to inspect arbitrary objects (e.g., the `uploads/` prefix from the reference upload route, or any orphaned keys).
