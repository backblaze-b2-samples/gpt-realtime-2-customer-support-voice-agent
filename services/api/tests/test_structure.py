...
def test_openai_only_in_repo():
    """Verify the openai SDK is only imported in app/repo/.

    The Realtime API is exposed to the browser via a thin token-mint
    endpoint; non-realtime calls (summary generation) happen in repo.
    No layer above repo should import `openai` directly.
    """
    violations = []
    for layer in LAYER_ORDER:
        if layer == "repo":
        ...
        for pyfile in _get_python_files(layer_dir):
            for imp in _get_imports(pyfile):
                if imp == "openai" or imp.startswith("openai."):
                    rel = pyfile.relative_to(APP_ROOT.parent)
                    violations.append(f"{rel}: openai imported outside repo/")
    assert violations == [], "openai boundary violations:\n" + "\n".join(violations)


def test_required_modules_exist():
    """Verify each new layer module is present.

    Catches regressions where a builder agent deletes a module the
    rest of the system depends on (eg the helpdesk adapter, the call
    orchestrator, the openai client).
    """
    required = [
        "repo/b2_client.py",
        "repo/b2_calls.py",
        "repo/openai_client.py",
        "repo/helpdesk_adapter.py",
        "service/call_orchestrator.py",
        "service/tool_executor.py",
        "runtime/realtime.py",
        "runtime/tools.py",
        "runtime/calls.py",
        "types/calls.py",
    ...
    missing = [r for r in required if not (APP_ROOT / r).exists()]
    assert missing == [], "Missing required modules:\n" + "\n".join(missing)
